 ...
 */
export function trackAndAdapt(userId: number, text: string, currentSessionLang: Locale): Locale {
  ...
  if (total >= 3) {
    ...
    let newLang: Locale = profile.language;
 ...
 */
export function setExplicitLanguage(userId: number, lang: Locale): void {
  const profile = loadProfile(userId);
  if (!profile) return;
