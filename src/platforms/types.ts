...
export interface IncomingMessage {
  /** Platform identifier */
  platform: "telegram" | "discord" | "whatsapp" | "slack" | "signal" | "web";
...
export interface PlatformAdapter {
  ...
  /** Send a text message. Returns the platform's message ID if available,
   *  which can be passed to editMessage() later for progress tickers. */
   ...
   *  Returns the new message ID if the platform assigns one, otherwise
   *  the original messageId. v4.12.0. */
  editMessage?(chatId: string, messageId: string, newText: string): Promise<string>;
