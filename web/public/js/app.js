...
function initUI() {
  ...
  const NAV = [
    ...
    { section: 'nav.data', items: [
      ...
      { page: 'workspaces', icon: 'compass', label: 'nav.workspaces' },
...
async function loadWorkspaces() {
  try {
    ...
    if (!data.workspaces || data.workspaces.length === 0) {
      list.innerHTML = `<div class="hint" style="color:var(--fg2)">No workspaces yet. Create one at <code>~/.alvin-bot/workspaces/&lt;name&gt;.md</code> with YAML frontmatter (purpose, cwd, emoji, color, channels). See <code>docs/install/slack-setup.md</code>.</div>`;
      return;
    }
    list.innerHTML = data.workspaces.map(ws => {
      const emoji = ws.emoji || '🧭';
      const color = ws.color || '#6366f1';
      const purpose = ws.purpose || '<em style="opacity:0.6">(no purpose set)</em>';
      const cost = (ws.stats.totalCost || 0).toFixed(4);
      const sessionCount = ws.stats.sessionCount || 0;
      const messageCount = ws.stats.messageCount || 0;
      const channelsList = (ws.channels && ws.channels.length > 0)
        ? `<div class="sub" style="font-size:0.75em;opacity:0.6;margin-top:4px">channels: ${ws.channels.join(', ')}</div>`
        : '';
      return `
        <div class="list-item" style="border-left:4px solid ${color};padding-left:12px">
          <div class="info" style="flex:1">
            <div class="name" style="font-weight:600">${emoji} ${ws.name}</div>
            <div class="desc">${purpose}</div>
            <div class="sub" style="font-size:0.78em;opacity:0.7;margin-top:2px">cwd: <code>${ws.cwd}</code></div>
            ${channelsList}
          </div>
          <div style="text-align:right;font-family:monospace;font-size:0.85em;opacity:0.8;line-height:1.5">
            <div>${sessionCount} session${sessionCount === 1 ? '' : 's'}</div>
            <div>${messageCount} msg</div>
            <div>$${cost}</div>
          </div>
        </div>`;
    }).join('');
  } catch (err) {
    console.error('loadWorkspaces failed:', err);
  }
}
